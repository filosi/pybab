...
-- LABEL
------------------------------------------------------------------------------------------
DROP TYPE IF EXISTS labeltype CASCADE;
CREATE TYPE labeltype AS ENUM ('default','administrative');

DROP TABLE IF EXISTS gt_label CASCADE;
CREATE TABLE IF NOT EXISTS gt_label (
    id BIGSERIAL PRIMARY KEY,
    name character varying(255) NOT NULL UNIQUE,
    type labeltype NOT NULL DEFAULT 'default'
);
------------------------------------------------------------------------------------------
...
CREATE FOREIGN DATA WRAPPER pgsql VALIDATOR postgresql_fdw_validator;
------------------------------------------------------------------------------------------
-- FOREIGN TABLE
------------------------------------------------------------------------------------------
/*DROP TABLE IF EXISTS gt_foreign_table CASCADE;
CREATE TABLE IF NOT EXISTS gt_foreign_table (
	id BIGINT NOT NULL PRIMARY KEY REFERENCES gt_catalog(id),
	fdw_type text DEFAULT 'pgsql' 
...
INSERT INTO gt_statistical_group VALUES (1,'temp');
------------------------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS  gt_catalog_statistical (
	...
	group_id BIGINT NOT NULL DEFAULT 0 REFERENCES gt_statistical_group(id)
...
insert into gt_statistical_tree values (1,1,0);
------------------------------------------------------------------------------------------
-- CATALOG STATISTICAL METADATA
------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS gt_statistical_meta CASCADE;
CREATE TABLE IF NOT EXISTS gt_statistical_meta (
	id BIGSERIAL PRIMARY KEY,
	statistical_id BIGINT UNIQUE NOT NULL REFERENCES gt_catalog_statistical(id)
		ON UPDATE CASCADE ON DELETE CASCADE,
	title varchar(255),
	description TEXT,
	category TEXT,
...
INSERT INTO gt_indicator_group VALUES (1,'temp');
------------------------------------------------------------------------------------------
-- INDICATOR GROUP TREE (triggers are below)
------------------------------------------------------------------------------------------
...
insert into gt_indicator_tree values (1,1,0);
------------------------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS gt_catalog_indicator (
	id BIGINT NOT NULL UNIQUE,
	group_id BIGINT NOT NULL DEFAULT 0 REFERENCES gt_indicator_group(id)
		ON UPDATE CASCADE ON DELETE CASCADE,
	tableschema name NOT NULL,
	tablename name NOT NULL,
	code_column name NOT NULL,
...
-- INDICATOR
------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS gt_indicator CASCADE;
CREATE TABLE IF NOT EXISTS gt_indicator (
	id BIGINT NOT NULL UNIQUE,
	group_id BIGINT NOT NULL DEFAULT 0 REFERENCES gt_indicator_group(id)
		ON UPDATE CASCADE ON DELETE CASCADE,
	name VARCHAR(255) NOT NULL,
	function_name name NOT NULL,
	function_schema name NOT NULL DEFAULT 'public',
	function_parameters text NOT NULL,
	gs_layer TEXT NOT NULL,
	gs_workspace TEXT,
	gs_url TEXT NOT NULL,
	gs_style TEXT,
	gs_style_parameters TEXT
);
------------------------------------------------------------------------------------------
-- INDICATOR <--> LABEL
------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS gt_indicator_label_link CASCADE;
CREATE TABLE IF NOT EXISTS gt_indicator_label (
	id BIGSERIAL NOT NULL PRIMARY KEY,
	gt_indicator_id BIGINT NOT NULL REFERENCES gt_indicator(id)
		ON UPDATE CASCADE ON DELETE CASCADE,
	gt_label_id BIGINT NOT NULL REFERENCES gt_label(id)
		ON UPDATE CASCADE ON DELETE CASCADE,
	UNIQUE (gt_indicator_id,gt_label_id)
);
------------------------------------------------------------------------------------------
-- INDICATOR METADATA
------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS gt_indicator_meta CASCADE;
CREATE TABLE IF NOT EXISTS gt_indicator_meta (
	id BIGSERIAL PRIMARY KEY,
	indicator_id BIGINT UNIQUE NOT NULL REFERENCES gt_indicator(id)
		ON UPDATE CASCADE ON DELETE CASCADE,
	title varchar(255),
	description TEXT,
	category TEXT,
...
CREATE OR REPLACE FUNCTION gt_catalog_statistical_after_insert_check() RETURNS TRIGGER AS
$BODY$
DECLARE
	colname name;
	datatype varchar;
...
BEGIN
	...
	LOOP
		IF (NEW.code_column = colname AND datatype = 'character varying'::varchar) THEN
			codefound := true;
		END IF;
		IF (NEW.data_column = colname) THEN
			datafound := true;
		END IF;
	END LOOP;
...
BEGIN
	IF (TG_OP = 'DELETE') THEN
		...
		--DELETE FROM gt_meta WHERE gt_catalog_id = OLD.id;
		RETURN OLD;
	END IF;

	IF (TG_OP = 'UPDATE') THEN
		UPDATE gt_element_catalog_link SET gt_catalog_id = NEW.id WHERE gt_catalog_id = OLD.id;
		--UPDATE gt_meta SET gt_catalog_id = NEW.id WHERE gt_catalog_id = OLD.id;
		RETURN NEW;
	END IF;
END
$BODY$
...
CREATE TRIGGER gt_catalog_statistical_after_check AFTER INSERT ON gt_catalog_statistical
	FOR EACH ROW EXECUTE PROCEDURE gt_catalog_statistical_after_insert_check();
...
CREATE TABLE IF NOT EXISTS gt_meta (
	id BIGSERIAL NOT NULL PRIMARY KEY,
	gt_catalog_id BIGINT UNIQUE NOT NULL,
	title varchar(255),
	description TEXT,
	category TEXT,
	extent TEXT,
...
DROP TRIGGER IF EXISTS gt_meta_check ON gt_meta CASCADE;
CREATE TRIGGER gt_meta_insert_check BEFORE INSERT OR UPDATE ON gt_meta
	FOR EACH ROW EXECUTE PROCEDURE gt_element_catalog_insert_update_check();*/
